...
public class B_LoaderTempTest {
    ...
    public void tempFile() throws Exception {
        ...
        File sourceRen = new File(sourceFile + ".old");

        source.renameTo(sourceRen);

        assertFalse(source.exists());

        assertNull(Loader.getTempDir());

        Loader.loadLibrary();

        assertNotNull(Loader.getTempDir());

        sourceRen.renameTo(source);
    }
}
